...
Save previews to `.Liuyan-frontend-slides/slide-previews/` (style-a.html, style-b.html, style-c.html). Each should be self-contained and compact, showing one animated title slide.
...
### ⚠️ Common Generation Pitfalls (NON-NEGOTIABLE — learned from real failures)

Two recurring breakages happen when the supporting reference files are NOT read **in full** before generating. Both are **silent**: the page renders and looks fine at a glance, but is functionally broken. Rule: read viewport-base.css + html-template.md + animation-patterns.md **COMPLETELY** and copy them into the file — do NOT hand-roll a "simplified" version from memory.

**PITFALL 1 — Content is clipped / "the slide only shows part of itself":**
- ❌ WRONG: `body { width: 1920px; overflow: hidden; }` with slide content laid out at 1920px. On any viewport narrower than 1920px (preview panes, laptops), the page overflows the window and is cut off on the right/bottom — the user sees an incomplete slide.
- ✅ RIGHT: use the three-layer fixed stage from `viewport-base.css` **verbatim**: `.deck-viewport` (fills the window) → `#deckStage.deck-stage` (fixed 1920×1080, scaled as a whole by JS `transform: scale()` from `deck-stage.js`) → `.slide`. The whole stage scales uniformly and centers; nothing is ever clipped or reflowed. Copy `viewport-base.css` into the file as-is. Do not reinvent the scaling.

**PITFALL 2 — "E" key / hover-hotzone editing does not work:**
- ❌ WRONG: implementing only a single "edit" button, or omitting the edit trigger entirely. The user expects the documented behavior: press **E** (or hover the top-left hotzone) to enter edit mode.
- ✅ RIGHT: implement editing by **combining** two sources — the E-key toggle + top-left `.edit-hotzone` + `.edit-toggle` scaffolding from `html-template.md`, and the full `InlineEditing` class (floating `.inline-toolbar` with font-size / text color / bg color / drag-to-move, **M** for move mode, Ctrl/Cmd+S save) from the "Inline Editing & Save" section of this file. Do not downgrade to a button-only control.

**Pre-generation self-check (do not skip):**
1. `viewport-base.css` full contents are inside `<style>` (the `.deck-viewport` / `.deck-stage` / `.slide` rules, not a custom body-width fix) → prevents PITFALL 1.
2. E-key handler (`e.key === 'e'`) + `.edit-hotzone` + floating toolbar exist in the JS → prevents PITFALL 2.
3. Fonts come from Fontshare/Google Fonts, not system fonts.
4. After writing, run a JS syntax check (e.g. `new Function(scriptText)`) so the page never white-screens.

### Inline Editing & Save (Default Feature)

Every generated HTML deck must support in-browser text editing and a one-click save workflow. Include it by default unless the user explicitly requests a locked/export-only file.

This means the **FULL module** (E-key toggle + top-left hover hotzone + floating toolbar with font-size/color/drag + M for move + Ctrl/Cmd+S save) — **NOT** a simplified button-only control. Build it by combining the E-key/hotzone scaffolding from `html-template.md` with the full `InlineEditing` class in the section below; see the Common Generation Pitfalls section for what breaks when this is skipped.

**Interaction design:**

- Press **E** or hover the top-left hotzone to toggle edit mode.
- In edit mode, **all slide text elements** become `contenteditable="true"` and show a dashed gold outline (or another obvious visual cue that matches the deck theme).
- Click any slide element to select it; a floating toolbar appears with controls for **font size** and **drag-to-move positioning**.
- Toggle move mode (toolbar button or **M**) to drag the selected element. Drag deltas are converted to the fixed 1920×1080 stage coordinate system so positions stay correct at any viewport size.
- A save button appears next to the edit toggle while edit mode is active.
- Also support **Ctrl+S / Cmd+S** as a save shortcut (must work even when a toolbar control is focused).

**Editable selector:**

Prefer a generic direct-text selector over a fixed allow-list, so labels, tags, stat numbers, chart values, kickers, and any future text class are all editable without maintaining a long selector list.

```js
toggleEditMode() {
    this.isActive = !this.isActive;
    this.toggle.classList.toggle('active', this.isActive);
    this.toggle.textContent = this.isActive ? '✓' : '✎';
    this.saveBtn.classList.toggle('show', this.isActive);

    const skipTags = new Set(['SCRIPT','STYLE','SVG','PATH','RECT','CIRCLE','IMG','VIDEO','AUDIO','IFRAME','INPUT','TEXTAREA','BUTTON']);
...
Provide a small fixed-position toolbar while an element is selected. It should at minimum support font-size adjustment, text color, element background color, and drag-to-move; reset is optional but recommended.
...
.inline-toolbar input[type="color"]::-webkit-color-swatch-wrapper {
    padding: 2px;
}
.inline-toolbar input[type="color"]::-webkit-color-swatch {
    border: none;
    border-radius: 3px;
}
.inline-selected {
...
applyFontSize() {
    ...
    const val = parseFloat(this.fontSizeInput.value);
    if (!isNaN(val) && val > 0) {
        this.selectedEl.style.fontSize = val + 'px';
    }
}

applyTextColor() {
    if (!this.selectedEl) return;
    this.selectedEl.style.color = this.textColorInput.value;
}

applyBgColor() {
    if (!this.selectedEl) return;
    this.selectedEl.style.backgroundColor = this.bgColorInput.value;
}

onMouseMove(e) {
    if (!this.drag.active || !this.selectedEl) return;
    ...
    const x = this.drag.origX + dx;
    const y = this.drag.origY + dy;
    this.selectedEl.dataset.translateX = x;
    this.selectedEl.dataset.translateY = y;
    if (window.getComputedStyle(this.selectedEl).position === 'static') {
        this.selectedEl.style.position = 'relative';
    }
    this.selectedEl.style.left = x + 'px';
    this.selectedEl.style.top = y + 'px';
}
```

**Save behavior:**

- On save, turn edit mode off, deselect the active element, and remove **all** `contenteditable` attributes (including `contenteditable="false"` left over from move mode).
- Inline style changes (font-size, color, background-color, left/top offsets) are preserved because they are part of the serialized HTML.
- Serialize the entire document with `document.documentElement.outerHTML`.
- Create a Blob of type `text/html` and trigger a download via a temporary `<a download>` element using `URL.createObjectURL`.
- The downloaded file name should match the original deck name (e.g. `NA6_FAQ_Launch_Meeting.html`).
- Because browsers cannot overwrite a local file from a `file://` web page, the user must manually replace the original HTML file with the downloaded one.
...
.edit-save {
    ...
    height: 44px;
    border-radius: 50%;
    background: rgba(200, 168, 112, 0.9);
    border: 1px solid var(--gold);
    color: #1a2030;
    font-size: 20px;
    cursor: pointer;
...
save() {
    ...
    this.saveBtn.classList.remove('show');
    ...
    a.href = url;
    a.download = 'NA6_FAQ_Launch_Meeting.html';
...
1. **Clean up** — Delete `.Liuyan-frontend-slides/slide-previews/` if it exists
2. **Open** — Use `open [filename].html` to launch in browser
3. **Summarize** — Tell the user:
   - File location, style name, slide count
   - Navigation: Arrow keys, Space, swipe/tap if enabled
   - How to customize: `:root` CSS variables for colors, font link for typography, `.reveal` class for animations
   - Inline editing is available: Hover top-left corner or press E to enter edit mode; click any text to edit; use the floating toolbar or M key to drag and reposition elements; change font size from the toolbar; press Ctrl+S / Cmd+S to save
   - Offer the natural post-draft actions: ask for revisions, edit text directly in the browser, adjust layout/typography, or export/share
